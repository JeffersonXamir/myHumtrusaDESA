...
<%@ attribute
	...
	required="false"
	description="
(String)The base CSS class to apply to this panels element (defaults to x-panel).
" %>
<%@ attribute
	name="bbar"
...
<%@ attribute
	...
	required="false"
	description="
(Number)The number of clicks on a cell required to display the cells editor (defaults to 2)
" %>
<%@ attribute
	name="cls"
	type="java.lang.String"
...
<%@ attribute
	...
	required="false"
	description="
(Object)A config object that will be applied to the grids UI view. Any of the config options available for Ext.grid.GridView can be specified here.
" %>
<%@ attribute
	name="width"
	type="java.lang.Integer"
...
<%@ attribute
	...
	required="false"
	description="
( Grid this, Object sortInfo )Fires when the grids store sort changes
" %>
<%@ attribute
	name="onStaterestore"
	type="java.lang.Object"
...
<%@ tag import="org.apache.commons.beanutils.BeanUtils" dynamic-attributes="dynamicAttributes" description="Class for creating and editable grid. " %>
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" include="*" exclude="items,cm,plugins,view" tagJspContext='<%=jspContext %>' dynamicAttributes="${dynamicAttributes}" />	
<jsp:doBody /><c:set var="item">new Ext.grid.EditorGridPanel({<c:choose><c:when test="<%= BeanUtils.getProperty(this,"cm")!=null %>">cm:new Ext.grid.ColumnModel(<%= BeanUtils.getProperty(this,"cm") %>),</c:when><c:otherwise>cm:new Ext.grid.ColumnModel([<%= BeanUtils.getProperty(this,"colModel") %>]),</c:otherwise></c:choose><c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach><% String data = BeanUtils.getProperty(this,"data");String fields = BeanUtils.getProperty(this,"fields"); %><c:if test='<%= data!=null %>'>store:new Ext.data.SimpleStore({fields: [<%= fields %>],data: [<%= data.substring(0,data.length()-1) %>]}),</c:if><c:if test='<%= BeanUtils.getProperty(this,"tbar")!=null %>'>tbar:[<% String tbar = BeanUtils.getProperty(this,"tbar");jspContext.getOut().write(tbar.substring(0,tbar.length()-1));%>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"sm")!=null %>'>sm:<%=BeanUtils.getProperty(this,"sm") %>,</c:if><c:if test='<%= BeanUtils.getProperty(this,"store")!=null %>'>store:<%=BeanUtils.getProperty(this,"store") %>,</c:if><c:if test='<%= BeanUtils.getProperty(this,"bbar")!=null %>'>bbar:[<% String bbar = BeanUtils.getProperty(this,"bbar");jspContext.getOut().write(bbar.substring(0,bbar.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"buttons")!=null %>'>buttons:[<% String buttons = BeanUtils.getProperty(this,"buttons");jspContext.getOut().write(buttons.substring(0,buttons.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"tools")!=null %>'>tools:[<% String tools = BeanUtils.getProperty(this,"tools");jspContext.getOut().write(tools.substring(0,tools.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"viewConfig")!=null %>'>viewConfig:<%= BeanUtils.getProperty(this,"viewConfig") %>,</c:if><c:if test='<%= BeanUtils.getProperty(this,"view")!=null %>'>view:<%= BeanUtils.getProperty(this,"view") %>,</c:if><c:if test='<%= BeanUtils.getProperty(this,"plugins")!=null %>'>plugins:<%= BeanUtils.getProperty(this,"plugins") %>,</c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
