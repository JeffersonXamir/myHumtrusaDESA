...
<%@ attribute
	...
	type="java.lang.String"
	required="false"
	description="(String) Type of a toolbar ('tbar','bbar','body'), default is 'body'."
 %>
<%@ attribute
	name="url"
	type="java.lang.String"
	required="false"
	description="(String) Url donde se toma el xml."
 %>
...
<jsp:doBody/><c:set var="item">new Ext.ux.MenuPerfiles({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach>items:[<% String items = BeanUtils.getProperty(this,"items");if(items!=null){jspContext.getOut().write(items.substring(0,items.length()-1));}%>],listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
<%	
