...
<%@ tag import="org.apache.commons.beanutils.BeanUtils" dynamic-attributes="dynamicAttributes" description="Provee un data picker" %>
<c:if test="${empty(value)}"><jsp:doBody var="value" scope="page" /></c:if>
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" include="*" exclude="items,maskRe" tagJspContext="<%=jspContext %>" dynamicAttributes="${dynamicAttributes}" />	
<c:set var="item">new Ext.form.ChooserField({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}<c:if test="${onTriggerClick != '' && onTriggerClick != null}">,onTriggerClick: function(){<c:out value="${onTriggerClick}" escapeXml="false"></c:out>}</c:if><c:if test="${maskRe!='' && maskRe!=null}">,maskRe:<c:out value="${maskRe}" escapeXml="false"></c:out></c:if>}),</c:set>
