<%@ attribute
	...
	required="false"
	description="
(Boolean/Object)If passed, this stores load method is automatically called after creation with the autoLoad object
" %>
<%@ attribute
	name="baseParams"
	type="java.lang.Object"
...
<%@ attribute
	...
	required="false"
	description="
(Array)Inline data to be loaded when the store is initialized.
" %>
<%@ attribute
	name="listeners"
	type="java.lang.Object"
...
If remote sorting is specified, then clicking on a column header causes the current page to be requested from the server with the addition of the following two parameters:
    ...
    * sort : String
      ...
      The name (as specified in the Records Field definition) of the field to sort on.
    * dir : String
...
<%@ attribute
	...
	required="false"
	description="
()Fires if an exception occurs in the Proxy during loading.Called with the signature of the Proxys loadexception event.
" %>
<%@ attribute
	name="onMetachange"
	type="java.lang.Object"
	required="false"
	description="
( Store this, Object meta )Fires when this stores reader provides new metadata (fields). This is currently only supported for JsonReaders.
" %>
<%@ attribute
	name="onRemove"
	type="java.lang.Integer"
...
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" include="*" javaScript="baseParams,data,proxy,reader,sortInfo" tagJspContext='<%=jspContext %>' dynamicAttributes="${dynamicAttributes}" />	
<jsp:doBody/><c:set var="id"><extutil:getTagId tag='<%=this %>'></extutil:getTagId></c:set><c:set var="item">${id} = new Ext.data.Store({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach><c:if test='<%= BeanUtils.getProperty(this,"reader")!=null %>'>reader:<%= BeanUtils.getProperty(this,"reader") %>,</c:if><c:if test='<%= BeanUtils.getProperty(this,"proxy")!=null %>'>proxy:<%= BeanUtils.getProperty(this,"proxy") %></c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}});</c:set>
<extutil:setExtCommons>${item}</extutil:setExtCommons><extutil:setParentProperties tag='<%= this %>' store="${id}" />