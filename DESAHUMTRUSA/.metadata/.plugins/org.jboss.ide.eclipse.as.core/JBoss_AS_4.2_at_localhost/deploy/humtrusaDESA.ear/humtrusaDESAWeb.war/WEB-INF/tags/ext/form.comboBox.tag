...
<%@ attribute
	...
	required="false"
	description="
(String)If specified, a hidden form field with this name is dynamically generated to store the fields data value (defaults to the underlying DOM elements name). Required for the combos value to automatically post during a form submission.
" %>
<%@ attribute
	name="hideLabel"
	type="java.lang.Boolean"
...
<%@ attribute
	...
	required="false"
	description="
(RegExp)An input mask regular expression that will be used to filter keystrokes that dont match (defaults to null)
" %>
<%@ attribute
	name="maxHeight"
	type="java.lang.Integer"
...
x-combo-list-item
 ...
 to the templates container element. 

The template must contain one or more substitution parameters using field names from the Combos Store. An example of a custom template would be adding an
...
<%@ attribute
	...
	required="false"
	description="
( Object queryEvent )Fires before all queries are processed. Return false to cancel the query or set the queryEventscancel property to true.
" %>
<%@ attribute
	name="onBeforerender"
	type="java.lang.String"
...
<jsp:doBody />
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" include="*" exclude="items,tpl" tagJspContext='<%=jspContext %>' dynamicAttributes="${dynamicAttributes}" />	
<c:set var="item">new Ext.form.ComboBox({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach><c:if test='<%= BeanUtils.getProperty(this,"items")!=null %>'>mode:'local',displayField:'text',valueField:'value',triggerAction: 'all',store:new Ext.data.SimpleStore({fields: ['value', 'text'],data: [<%= BeanUtils.getProperty(this,"items").substring(0,items.length()-1) %>]}),</c:if><c:if test='<%= BeanUtils.getProperty(this,"store")!=null %>'>store: <%= BeanUtils.getProperty(this,"store") %>,</c:if><c:if test='<%= BeanUtils.getProperty(this,"tpl")!=null %>'>tpl: <%= BeanUtils.getProperty(this,"tpl") %>,</c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
