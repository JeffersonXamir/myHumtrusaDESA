...
<%@ attribute
	...
	required="false"
	description="
(String)The base CSS class applied to the fieldset (defaults to x-fieldset).
" %>
<%@ attribute
	name="bbar"
...
<%@ attribute
	...
	required="false"
	description="
(String)The name to assign to the fieldsets checkbox if checkboxToggle = true (defaults to [checkbox id]-checkbox).
" %>
<%@ attribute
	name="checkboxToggle"
	type="java.lang.Boolean"
	required="false"
	description="
(Boolean)True to render a checkbox into the fieldset frame just in front of the legend (defaults to false). The fieldset will be expanded or collapsed when the checkbox is toggled.
" %>
<%@ attribute
	name="cls"
	type="java.lang.String"
...
<%@ attribute
	...
	required="false"
	description="
(String)The Ext.Container.layout to use inside the fieldset (defaults to form).
" %>
<%@ attribute
	name="layoutConfig"
	type="java.lang.Object"
...
<%@ tag import="org.apache.commons.beanutils.BeanUtils" dynamic-attributes="dynamicAttributes" description="Standard container used for grouping form fields." %>
...
<jsp:doBody/><c:set var="item">new Ext.form.FieldSet({autoHeight:true,autoWidth:true,<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach><c:if test='<%= BeanUtils.getProperty(this,"items")!=null %>'>items:[<% String items = BeanUtils.getProperty(this,"items");jspContext.getOut().write(items.substring(0,items.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"tbar")!=null %>'>tbar:[<% String tbar = BeanUtils.getProperty(this,"tbar");jspContext.getOut().write(tbar.substring(0,tbar.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"bbar")!=null %>'>bbar:[<% String bbar = BeanUtils.getProperty(this,"bbar");jspContext.getOut().write(bbar.substring(0,bbar.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"buttons")!=null %>'>buttons:[<% String buttons = BeanUtils.getProperty(this,"buttons");jspContext.getOut().write(buttons.substring(0,buttons.length()-1)); %>],</c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
