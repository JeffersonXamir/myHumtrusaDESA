...
<%@ attribute
	...
	required="false"
	description="
(Object)Opciones para la validacion remota. Autor:vandrade
" %>
<%@ attribute
	name="allowBlank"
...
<c:if test="${empty(value)}"><jsp:doBody var="value" scope="page" /></c:if>
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" include="*" exclude="items,rvOptions,maskRe" tagJspContext='<%=jspContext %>' dynamicAttributes="${dynamicAttributes}" />	
<c:set var="item">new Ext.form.TextField({<c:if test="<%= BeanUtils.getProperty(this,"rvOptions")!=null %>">rvOptions:<% String rvOptions = BeanUtils.getProperty(this,"rvOptions");jspContext.getOut().write(rvOptions.substring(0,rvOptions.length())); %>,</c:if><c:if test="<%= BeanUtils.getProperty(this,"maskRe")!=null %>">maskRe:<% String maskRe = BeanUtils.getProperty(this,"maskRe");jspContext.getOut().write(maskRe.substring(0,maskRe.length())); %>,</c:if><c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
