...
<%@ attribute
	...
	required="false"
	description="
(Boolean)True if this item can be visually activated (defaults to true)
" %>
<%@ attribute
	name="cls"
	type="java.lang.String"
...
<%@ attribute
	...
	required="false"
	description="
(String)The default CSS class to use for menu items (defaults to x-menu-item)
" %>
<%@ attribute
	name="listeners"
...
<%@ attribute
	...
	type="java.lang.String"
	required="false"
	description="(Mixed) Standard menu attribute consisting of a reference to a menu object, a menu id or a menu config blob (defaults to undefined).            "
 %>
...
<jsp:doBody/><c:set var="item">new Ext.menu.Item({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach><c:if test='<%=BeanUtils.getProperty(this,"menu")!=null %>'>menu:<%=BeanUtils.getProperty(this,"menu")%>,</c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
