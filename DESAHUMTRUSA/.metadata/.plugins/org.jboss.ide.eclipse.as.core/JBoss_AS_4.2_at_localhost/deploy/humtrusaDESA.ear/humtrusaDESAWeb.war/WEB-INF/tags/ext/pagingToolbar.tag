...
<%@ attribute
	...
	required="false"
	description="
(Boolean)True to display the displayMsg (defaults to false)
" %>
<%@ attribute
	name="displayMsg"
	type="java.lang.String"
...
<%@ attribute
	...
	required="false"
	description="
(String)The message to display when no records are found (defaults to No data to display)
" %>
<%@ attribute
	name="height"
...
<%@ attribute
	...
	required="false"
	description="
(Number)The number of records to display per page (defaults to 20)
" %>
<%@ attribute
	name="plugins"
	type="java.lang.Object"
...
<%@ attribute
	...
	required="false"
	description="
(Ext.data.Store)The Ext.data.Store the paging toolbar should use as its data source (required).
" %>
<%@ attribute
	name="style"
	type="java.lang.String"
...
<%@ attribute
	...
	type="java.lang.String"
	required="false"
	description="(Mixed) A single item, or an array of items to be added to this container. Each item can be any type of object based on Ext.Component, or a valid config object for such an item. If a single item is being passed, it should be passed directly as an object reference (e.g., items: {...}). Multiple items should be passed as an array of objects (e.g., items: [{...}, {...}]). "
 %>
<%@ attribute
	name="toolbarType"
	type="java.lang.String"
	required="false"
	description="(String) Type of toolbar (tbar,bbar,body), default is body."
 %>
<%@ include file="inc/taglibs.jsp" %>
<%@ tag import="org.apache.commons.beanutils.BeanUtils" dynamic-attributes="dynamicAttributes" description="Basic Toolbar tag." %>
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" include="*" exclude="items,toolbarType" tagJspContext='<%=jspContext %>' dynamicAttributes="${dynamicAttributes}" />	
<jsp:doBody/>
<c:set var="store"><extutil:getParentProperty property="store" tag='<%=this %>'></extutil:getParentProperty></c:set>
<c:set var="item">new Ext.PagingToolbar({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach>items:[<%String items = BeanUtils.getProperty(this,"items");if(items!=null){jspContext.getOut().write(items.substring(0,items.length()-1));}%>],store:${store},listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
<%	
	String toolbarType = (String) jspContext.getAttribute("toolbarType");
	String item = (String)jspContext.getAttribute("item");
	if(toolbarType==null || "body".equals(toolbarType)) toolbarType= "items";
%>
<extutil:setParentProperty tag='<%= this %>' property='<%= toolbarType %>' value="${item}" />