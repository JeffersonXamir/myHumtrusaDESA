...
<%@ attribute
	...
	required="false"
	description="
( Object This, Object node, Object callback )Fires before a network request is made to retrieve the Json text which specifies a nodes children.
" %>
...
<%@ tag 
	...
	description="A TreeLoader provides for lazy loading of an Ext.tree.TreeNodes child nodes from a specified URL. The response must be a JavaScript Array definition whose elements are node definition objects. A server request is sent, and child nodes are loaded only when a node is expanded. The loading nodes id is passed to the server under the parameter name node to enable the server to produce the correct child nodes.To pass extra parameters, an event handler may be attached to the beforeload event, and the parameters specified in the TreeLoaders baseParams property. This would pass an HTTP parameter called category to the server containing the value of the Nodes category attribute." %>
