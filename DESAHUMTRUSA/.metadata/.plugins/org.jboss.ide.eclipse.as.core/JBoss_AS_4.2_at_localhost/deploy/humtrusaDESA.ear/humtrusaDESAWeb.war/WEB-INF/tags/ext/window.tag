...
<%@ attribute
	...
	required="false"
	description="
(String)The base CSS class to apply to this panels element (defaults to x-window).
" %>
<%@ attribute
	name="bbar"
...
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" javaScript="autoLoad,keys,tools,manager" include="*" exclude="bbar" tagJspContext='<%= jspContext %>' dynamicAttributes="${dynamicAttributes}" />
<jsp:doBody var="jspBody" scope="page" />
<div class="x-hide-display" id="${id}El">${jspBody}</div><c:set var="item">new Ext.Window({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach><c:if test='<%= BeanUtils.getProperty(this,"items")!=null %>'>items:[<% String items = BeanUtils.getProperty(this,"items");jspContext.getOut().write(items.substring(0,items.length()-1));%>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"tbar")!=null %>'>tbar:[<% String tbar = BeanUtils.getProperty(this,"tbar");jspContext.getOut().write(tbar.substring(0,tbar.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"bbar")!=null %>'>bbar:[<%String bbar = BeanUtils.getProperty(this,"bbar");jspContext.getOut().write(bbar.substring(0,bbar.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"buttons")!=null %>'>buttons:[<%String buttons = BeanUtils.getProperty(this,"buttons");jspContext.getOut().write(buttons.substring(0,buttons.length()-1));%>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"tools")!=null %>'>tools:[<%String tools = BeanUtils.getProperty(this,"tools");jspContext.getOut().write(tools.substring(0,tools.length()-1));%>],</c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(){')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(){')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>},contentEl:'${id}El'})</c:set>
<c:set var="parentClass"><extutil:getParentTagClass tag='<%= this%>'/></c:set>
<c:if test="${parentClass=='viewport_tag' || parentClass=='body_tag' || parentClass=='bodyInterno_tag'}"><extutil:setExtCommons>${id} = ${item};</extutil:setExtCommons></c:if>
<c:if test="${parentClass!='viewport_tag' && parentClass!='body_tag' && parentClass!='bodyInterno_tag'}"><extutil:setParentProperties tag='<%=this%>' items="${item}," /></c:if>