...
Ext.ns('Etech.componentes.AgrupadorAvanzadoEmpleados');

Etech.componentes.AgrupadorAvanzadoEmpleados = Ext.extend(Ext.form.Field, {
	defaultAutoCreate : {
		tag : 'input',
		type : 'hidden'
	},
	...
	initComponent : function() {
		...
		if (typeof window.event == 'undefined') {
			document.onkeypress = function(e) {
				...
				if ((test_var == 'INPUT' && test_type == 'TEXT')
						|| test_var == 'TEXTAREA') {
					return e.keyCode;
				} else if (e.keyCode == 8) {
					e.preventDefault();
		...
		Etech.componentes.AgrupadorAvanzadoEmpleados.superclass.initComponent
				.call(this);
...
/* ADDED BY VANDRADE */
		...
		this.abrirVentana = function() {
			if (!this.disabled) {
				...
				var chkIncluirCesantes = new Ext.form.Checkbox({
							hideLabel : true,
							boxLabel : "Incluir Cesantes"
							,
						});
				...
				txtCodBusqueda.on("specialkey", function() {
					...
					storeVentanaPrincipal.baseParams.incluircesantes = (chkIncluirCesantes
							.getValue() ? "S" : "N");
				...
				var confVentana = {
					...
					tbar : [{
							...
							// {xtype:'tbtext',text:'Criterio Secundario:'},
							// botonFiltroAvanzado
							chkIncluirCesantes,
	...
	onRender : function(ct, posicion) {
		...
		Etech.componentes.AgrupadorAvanzadoEmpleados.superclass.onRender.call(
				this, ct, posicion);
...
/* Registro del xtype */
Ext.reg('xagrupadoravanzadoempleados',
		Etech.componentes.AgrupadorAvanzadoEmpleados);
