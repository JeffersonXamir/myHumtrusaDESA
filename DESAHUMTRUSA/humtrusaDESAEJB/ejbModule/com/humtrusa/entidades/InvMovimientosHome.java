...
public class InvMovimientosHome {
	...
	public InvMovimientos merge(InvMovimientos detachedInstance) {
		...
		try {
			InvMovimientos result = (InvMovimientos) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public List findByExample(InvMovimientos instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("com.humtrusa.entidades.InvMovimientos")
