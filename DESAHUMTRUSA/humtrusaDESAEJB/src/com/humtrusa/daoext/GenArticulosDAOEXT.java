...
public class GenArticulosDAOEXT {
	...
	public Genarticulos obtenerArticulo(long codEmpresa, long codarticulo) {
		Genarticulos articulo  = null;
		Session objSession=HibernateSessionFactory.getSession();
		Query query= null;
		try {
			query=objSession.createQuery("select ga from Genarticulosxempresa axe " + 
										"inner join axe.genarticulos ga " + 
										"where axe.genarticulos.codarticulo =:articulo and axe.genempresas.codempresa =:empresa and axe.genestados.codestado = 1");
					query.setParameter("articulo", codarticulo);
					query.setLong("empresa", codEmpresa);
			articulo = (Genarticulos) query.uniqueResult();
		} catch (Exception e) {
			e.printStackTrace();
		}
