...
<jsp:doBody/><c:set var="item">new Ext.data.XmlReader({<c:forEach items="${configMap}" var="config"><c:if test="${config.key!='fields'}">${config.key}:${config.value},</c:if></c:forEach><c:if test='<%= BeanUtils.getProperty(this,"fields")!=null %>'>fields:[<%= BeanUtils.getProperty(this,"fields") %>],</c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}})</c:set>
