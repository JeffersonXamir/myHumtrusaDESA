...
<%@ attribute
	...
	required="false"
	description="
(String)Valid values are left, center and right (defaults to center)
" %>
<%@ attribute
	name="buttons"
	type="java.lang.String"
...
<%@ attribute
	...
	required="false"
	description="
(String)A css class to apply to the x-form-item of fields. This property cascades to child containers.
" %>
<%@ attribute
	name="items"
	type="java.lang.String"
...
<%@ attribute
	...
	required="false"
	description="
(String)Valid values are left, top and right (defaults to left). This property cascades to child containers if not set.
" %>
<%@ attribute
	name="labelWidth"
	type="java.lang.Integer"
...
<%@ attribute
	...
	type="java.lang.String"
	required="false"
	description="(String)The request method to use (GET or POST) for form actions if one isnt supplied in the action options."
 %>
<%@ attribute
	name="reader"
	type="java.lang.String"
...
<%@ attribute
	...
	type="java.lang.String"
	required="false"
	description="(String)The URL to use for form actions if one isnt supplied in the action options."
 %>
<%@ attribute
...
<c:if test="${empty(id)}"><c:set var="id"><extutil:getTagId tag='<%= this %>' /></c:set></c:if>
<extutil:processTagAttributes configVar="configMap" eventsVar="eventsMap" include="*" exclude="items" javaScript="dragConfig,dropConfig,loader" tagJspContext='<%=jspContext %>' dynamicAttributes="${dynamicAttributes}" />	
<jsp:doBody/><c:set var="item">new Ext.form.FormPanel({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach><c:if test='<%= BeanUtils.getProperty(this,"items")!=null %>'>items:[<% String items = BeanUtils.getProperty(this,"items");jspContext.getOut().write(items.substring(0,items.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"tbar")!=null %>'>tbar:[<% String tbar = BeanUtils.getProperty(this,"tbar");jspContext.getOut().write(tbar.substring(0,tbar.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"bbar")!=null %>'>bbar:[<% String bbar = BeanUtils.getProperty(this,"bbar");jspContext.getOut().write(bbar.substring(0,bbar.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"buttons")!=null %>'>buttons:[<% String buttons = BeanUtils.getProperty(this,"buttons");jspContext.getOut().write(buttons.substring(0,buttons.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"tools")!=null %>'>tools:[<% String tools = BeanUtils.getProperty(this,"tools");jspContext.getOut().write(tools.substring(0,tools.length()-1)); %>],</c:if><c:if test='<%= BeanUtils.getProperty(this,"reader")!=null %>'>reader:<%=BeanUtils.getProperty(this,"reader") %>,</c:if><c:if test='<%= BeanUtils.getProperty(this,"errorReader")!=null %>'>errorReader:<%=BeanUtils.getProperty(this,"errorReader") %>,</c:if>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
