...
<%@ attribute
	...
	required="false"
	description="
(Number)The maximum height to allow when grow = true (defaults to 1000)
" %>
<%@ attribute
	name="growMin"
	type="java.lang.Integer"
	required="false"
	description="
(Number)The minimum height to allow when grow = true (defaults to 60)
" %>
<%@ attribute
	name="height"
	type="java.lang.Integer"
...
<%@ attribute
	...
	required="false"
	description="
(Boolean)True to prevent scrollbars from appearing regardless of how much text is in the field (equivalent to setting overflow: hidden, defaults to false)
" %>
<%@ attribute
	name="readOnly"
	type="java.lang.Boolean"
...
<c:set var="item">new Ext.form.TextArea({<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
