...
<%@ attribute
	...
	required="false"
	description="
(String)The default CSS class to use for text items (defaults to x-menu-text)
" %>
<%@ attribute
	name="listeners"
...
<%@ attribute
	...
	required="false"
	description="
(String)The text to display for this item (defaults to )
" %>
<%@ attribute
	name="xtype"
	type="java.lang.String"
...
<jsp:doBody var="text"/><c:set var="item">new Ext.menu.TextItem('${text}',{<c:forEach items="${configMap}" var="config">${config.key}:${config.value},</c:forEach>listeners:{<c:forEach items="${eventsMap}" var="event" varStatus="status"><c:if test="${fn:indexOf(event.value,'function(')==-1}">${event.key}:function(){${event.value}}</c:if><c:if test="${fn:indexOf(event.value,'function(')>-1}">${event.key}:${event.value}</c:if>${status.last?'':','}</c:forEach>}}),</c:set>
