 ...
 * 
 * @author Ing. Jozef Sakki
 * @date 22. March 2008
 * @version $Id: Ext.ux.grid.CellActions.js 253 2008-05-11 09:31:48Z jozo $
 * 
 * @license Ext.ux.grid.CellActions is licensed under the terms of the Open
 ...
 * 
 * .ux-cell-value { position:relative; zoom:1; } .ux-cell-actions {
 * position:absolute; right:0; top:-2px; } .ux-cell-actions-left { left:0;
 * top:-2px; } .ux-cell-action { width:16px; height:16px; float:left;
 * cursor:pointer; margin: 0 0 0 4px; } .ux-cell-actions-left .ux-cell-action {
 * margin: 0 4px 0 0; }
 */
 ...
 * @constructor
 * 
 * CellActions plugin causes that column model recognizes the config property
 * cellAcions that is the array of configuration objects for that column. The
 * documentationi follows.
 * 
 * THE FOLLOWING CONFIG OPTIONS ARE FOR COLUMN MODEL COLUMN, NOT FOR CellActions
 * ITSELF.
 * 
 * @cfg {Array} cellActions Mandatory. Array of action configuration objects.
 ...
 *      clicked. This function is called with same signature as action event and
 *      in its original scope. If you need to call it in different scope or with
 *      another signature use createCallback or createDelegate functions. Works
 *      for statically defined actions. Use callbacks configuration options for
 *      store bound actions.
 *  -
 ...
 *      iconIndex is configured. Use this if you want static icons that are not
 *      base on the values in the record.
 *  -
 * @cfg {String} qtipIndex Optional. Field name of the field of the grid store
 *      record that contains tooltip text. If configured, the tooltip texts are
 *      taken from the store.
 *  -
 * @cfg {String} tooltip Optional. Tooltip text to use as icon tooltip. It is
 *      ignored if qtipIndex is configured. Use this if you want static tooltips
 *      that are not taken from the store.
 *  -
...
Ext.ux.grid.CellActions = function(config) {
	...
	this.addEvents(
			/**
			 * @event action Fires when user clicks a cell action
			 * @param {Ext.grid.GridPanel}
			 *            grid
			 * @param {Ext.data.Record}
			 *            record Record containing data of clicked cell
			 * @param {String}
			 *            action Action clicked (equals iconCls);
			 * @param {Mixed}
			 *            value Value of the clicke cell
			 * @param {String}
			 *            dataIndex as specified in column model
			 * @param {Number}
			 *            rowIndex Index of row clicked
			 * @param {Number}
			 *            colIndex Incex of col clicked
			 */
			'action'
			/**
			 * @event beforeaction Fires when user clicks a cell action but
			 *        before action event is fired. Return false to cancel the
			 *        action;
			 * @param {Ext.grid.GridPanel}
			 *            grid
			 * @param {Ext.data.Record}
...
Ext.extend(Ext.ux.grid.CellActions, Ext.util.Observable, {
	...
	/**
	 * @cfg {Number} actionWidth Width of action icon in pixels. Has effect only
	 *      if align:'left'
	 */
	,
	actionWidth : 20
	...
	/**
	 * @cfg {String} align Set to 'left' to put action icons before the cell
	 *      text. (defaults to undefined, meaning right)
	 */
	...
	tpl : '<div class="ux-cell-value" style="padding-left:{padding}px">'
			...
			+ '</div>' + '<tpl if="\'left\'===align">{value}</tpl>' + '<div>'

	/**
	 * Called at the end of processActions. Override this if you need it.
	 * 
	 * @param {Object}
	 *            c Column model configuration object
	 * @param {Object}
	 *            data See this.processActions method for details
	 */
	,
	...
	init : function(grid) {
		...
		grid.afterRender = grid.afterRender.createSequence(this.onRenderGrid,
				this);
	...
	// {{{
	/**
	 * processes the actions configs from column model column, saves callbacks
	 * and creates template
	 * 
	 * @param {Object}
	 *            c column model config of one column
	 * @private
	 */
	,
	processActions : function(c) {
		...
		var data = {
			...
			padding : 'left' === this.align ? this.actionWidth
					* c.cellActions.length : 0,
			value : '{value}',
			actions : []
		};
	...
	onClick : function(e, target) {
		...
		if (false !== row && false !== col && record && dataIndex && action) {
			...
			if (this.callbacks && 'function' === typeof this.callbacks[action]) {
				this.callbacks[action](this.grid, record, action, value,
						row.rowIndex, col);
			}
			...
			if (true !== this.eventsSuspended
					&& false === this
							.fireEvent('beforeaction', this.grid, record,
									action, value, dataIndex, row.rowIndex, col)) {
				return;
			} else if (true !== this.eventsSuspended) {
				this.fireEvent('action', this.grid, record, action, value,
						dataIndex, row.rowIndex, col);
			}
